/*
 * @(#)EquiJoinToNestedLoopsJoin.java   1.0   Jun 3, 2014
 *
 ...
 * @version 1.0
 */
public class EquiJoinToNestedLoopsJoin extends AbstractRule {
    ...
    */
   public EquiJoinToNestedLoopsJoin() {
      super(RuleType.EQUI_TO_NESTEDLOOPS_JOIN,
            // Original pattern
            new Expression(new EquiJoin(), new Expression(new Leaf(0)), new Expression(new Leaf(1))),
            // Substitute pattern
            new Expression(new NestedLoopsJoin(), new Expression(new Leaf(0)), new Expression(new Leaf(1))));
   }
