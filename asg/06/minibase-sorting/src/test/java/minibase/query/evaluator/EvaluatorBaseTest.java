...
public class EvaluatorBaseTest extends BaseTest {
   ...
   protected static final Schema S_RESERVES = new SchemaBuilder()
         .addField("sid", DataType.INT, 4)
         .addField("bid", DataType.INT, 4)
         .addField("day", DataType.INT, 4)
   ...
   protected static final Schema S_SAILORS = new SchemaBuilder()
         ...
         .addField("rating", DataType.INT, 4)
         .addField("age", DataType.FLOAT, 4)
         .build();

   /** Maximum number of Sailors tuple to generate. */
   protected static final int MAX_TUPLES = 150;
   ...
   protected File createSailors() {
      ...
      for (int i = 0; i < MAX_TUPLES; i++) {
         ...
         EvaluatorBaseTest.S_SAILORS.setField(tuple, 0, i);
         EvaluatorBaseTest.S_SAILORS.setField(tuple, 1,
               EvaluatorBaseTest.SNAMES[this.getRandom().nextInt(EvaluatorBaseTest.SNAMES.length)]);
         EvaluatorBaseTest.S_SAILORS.setField(tuple, 2, this.getRandom().nextInt(11));
         EvaluatorBaseTest.S_SAILORS.setField(tuple, 3, this.getRandom().nextFloat() * 81.9f + 18.0f);
    ...
    */
   protected int evaluate(final Operator operator, final String label) {
      int count = 0;
