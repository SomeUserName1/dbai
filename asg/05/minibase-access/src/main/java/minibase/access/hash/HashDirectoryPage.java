...
/**
 * The directory page of a hash index.
 *
 * <pre>
 * ┌─────────────────┐
 * │HashDirectoryPage│
...
class HashDirectoryPage implements PageType {
   ...
   protected static final int NEXT_PAGE_POINTER_OFFSET = DiskManager.PAGE_SIZE - PageID.SIZE;
   ...
   private static final short WRITABLE_SIZE = WRITABLE_SIZE_OFFSET;
   ...
   static void writePageID(final Page< ? extends HashDirectoryPage> page, final int pos, final PageID pageID) {
      final int off = pos * PageID.SIZE;
      if (off + PageID.SIZE > page.readShort(WRITABLE_SIZE_OFFSET)) {
