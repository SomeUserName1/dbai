...
/**
 * A replacement policy decides, which memory page (frame) is evicted from the buffer, if
...
public interface ReplacementPolicy {
   /**
    * A frame (main-memory page) can be in one of three states. If a frame is pinned, its state
    * is {@link #PINNED}. Once the pin count of a frame drops to zero, the state of the
    * frame becomes {@link #UNPINNED}. Finally, if a frame is freed, its state changes to
    * {@link #FREE}.
    */
   enum PageState {
