...
public final class Convert {
   ...
   /**
    * Reads from the given byte array at the specified position, and converts it into a
    * byte.
    *
    * @param data
   ...
   /**
    * Reads from the given byte array at the specified position, and converts it into an
    * integer.
    *
    * @param data
    *           byte array to read from
   ...
   /**
    * Reads from the given byte array at the specified position, and converts it to a byte
    ...
    */
   public static void writeString(final byte[] data, final int pos, final String str,
         final int length) {
   ...
   public static Date readDate(final byte[] data, final int pos) {
      //TODO add TimeZone
      final long days = ((data[pos] & 0xFF) << 24 | (data[pos + 1] & 0xFF) << 16 | (data[pos + 2] & 0xFF) << 8) >> 8;
   ...
   public static void writeDate(final byte[] data, final int pos, final Date date) {
      ...
      final long days = localDate.toEpochDay();
      data[pos + 0] = (byte) (days >> 16);
      data[pos + 1] = (byte) (days >> 8);
      data[pos + 2] = (byte) days;
   }
   ...
   public static Time readTime(final byte[] data, final int pos) {
      final long millisSinceMidnight = (data[pos + 0] & 0xFF) << 32 | (data[pos + 1] & 0xFF) << 24
            | (data[pos + 2] & 0xFF) << 16 | (data[pos + 3] & 0xFF) << 8 | (data[pos + 4] & 0xFF) << 0;
   ...
   public static void writeTime(final byte[] data, final int pos, final Time time) {
      ...
      data[pos + 0] = (byte) (millisSinceMidnight >>> 32);
      data[pos + 1] = (byte) (millisSinceMidnight >>> 24);
      data[pos + 2] = (byte) (millisSinceMidnight >>> 16);
      data[pos + 3] = (byte) (millisSinceMidnight >>> 8);
      data[pos + 4] = (byte) (millisSinceMidnight >>> 0);
   }
   ...
   private static long read64(final byte[] data, final int pos) {
      return (data[pos + 0] & 0xFFL) << 56 | (data[pos + 1] & 0xFFL) << 48
            | (data[pos + 2] & 0xFFL) << 40 | (data[pos + 3] & 0xFFL) << 32
            | (data[pos + 4] & 0xFFL) << 24 | (data[pos + 5] & 0xFFL) << 16
